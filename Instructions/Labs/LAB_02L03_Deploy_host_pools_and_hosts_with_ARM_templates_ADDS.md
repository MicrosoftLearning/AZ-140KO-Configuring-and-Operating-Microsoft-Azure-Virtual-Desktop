...
1. **사용자 지정 배포** 블레이드로 돌아와서 다음 설정을 지정합니다(나머지는 기존 값을 그대로 유지).
   ...
   |위치|**Azure Virtual Desktop의 배포 준비(AD DS)** 랩에서 AD DS 도메인 컨트롤러를 호스트하는 Azure VM을 배포했던 Azure 지역의 이름|
...
1. **az140-cl-vm11**에 연결된 원격 데스크톱 세션으로 다시 전환하여 **원격 데스크톱** 창 오른쪽 **Azure Virtual Desktop** 레이블 옆에 있는 줄임표 아이콘을 클릭합니다. 그런 다음 드롭다운 메뉴에서 **Unsubscribe**를 클릭하고 확인하라는 메시지가 표시되면 **Continue**를 클릭합니다.
